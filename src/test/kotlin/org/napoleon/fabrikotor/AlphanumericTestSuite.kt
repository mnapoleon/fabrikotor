...
class AlphanumericTestSuite : BaseTestSuite() {
  ...
  fun longRangeWithStep(): Array<Array<Any>> =
    arrayOf(
        arrayOf(1L, 10L, 1L, listOf(1L, 2L, 3L, 4L, 5L, 6L, 7L, 8L, 9L, 10L)),
        arrayOf(1L, 10L, 2L, listOf(1L, 3L, 5L, 7L, 9L))
    )

  @Test(dataProvider = "longRangeWithStep")
  fun testLongRangeWithStep(min: Long, max: Long, step: Long, expectedResult: List<Long>) {
    if (step <= 0) {
      try {
  ...
  fun testGuid() {
    ...
    assert(guid.matches(Regex("\\w{8}-\\w{4}-\\w{4}-\\w{4}-\\w{12}")))
  }

  @Test
  fun testGuidList() {
    val defaultList = alpha.randomGuidList()
    assert(100 == defaultList.size)
    val customList = alpha.randomGuidList(8, 30)
    assert(30 == customList.size)
  }
